...
function OrgInviteCodeGenerate(props: OrgInviteCodeGenerateProps) {
    ...
    async function createInviteWithUserGroup() {
        ...
        if (res.status == 200) {
            ...
            toast.error(t('signups.toasts.generateError', { status: res.status, detail: res.data.detail }))
        }
    }
        ...
        <div className='flex space-x-2 pt-2'>
            <div className='flex bg-slate-100 w-full h-[140px] rounded-lg'>
                <div className='flex flex-col mx-auto'>
                    <h1 className='mx-auto pt-4 text-gray-600 font-medium'>{t('signups.inviteCodes.generate.linkedToUserGroup.title')}</h1>
                    <h2 className='mx-auto text-xs text-gray-600 font-medium'>{t('signups.inviteCodes.generate.linkedToUserGroup.description')}</h2>
                            ...
                            <div className='flex space-x-4 items-center'>
                                ...
                                <div className=''>
                                    ...
                                    >
                                        ...
                                        <span> {t('signups.inviteCodes.generate.linkedToUserGroup.generate')} </span>
                                    </button>
                                </div>
                            </div>}
                        {usergroups?.length == 0 &&
                            <div className='flex space-x-3 items-center text-xs pt-3'>
                                <span className='px-3 text-yellow-700 font-bold rounded-full py-1 mx-3'>{t('signups.inviteCodes.generate.noUserGroups')} </span>
                                <Link className='px-3 text-blue-700 font-bold rounded-full py-1 bg-blue-100 mx-1' target='_blank' href={getUriWithOrg(org.slug, '/dash/users/settings/usergroups')}>{t('signups.inviteCodes.generate.createUserGroup')} </Link>
                            </div>}
                    </div>
                </div>
            </div>
            <div className='flex bg-slate-100 w-full h-[140px] rounded-lg'>
                <div className='flex flex-col mx-auto'>
                    <h1 className='mx-auto pt-4 text-gray-600 font-medium'>{t('signups.inviteCodes.generate.normal.title')}</h1>
                    <h2 className='mx-auto text-xs text-gray-600 font-medium'>{t('signups.inviteCodes.generate.normal.description')}</h2>
                    <div className='mx-auto pt-4'>
                        ...
                        >
                            <Ticket className="w-4 h-4" />
                            <span> {t('signups.inviteCodes.generate.normal.generate')} </span>
                        </button>
                    </div>
                </div>
            </div>
